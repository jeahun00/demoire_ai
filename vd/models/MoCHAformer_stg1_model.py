...
class MoCHAformer_Stg1_Model(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.out_vis = self.net_g(self.lq, phase='train', is_train=self.opt['is_train'])
    ...
    def test(self):
        ...
        _, _, h_old, w_old = self.gt.size()

        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            # self.net_f.eval()
            with torch.no_grad():
                self.output, self.out_vis = self.net_g_ema(self.lq, phase='val', is_train=self.opt['is_train'])
            ...
            # self.net_f.eval()
            with torch.no_grad():
                self.output, self.out_vis = self.net_g(self.lq, phase='val', is_train=self.opt['is_train'])
                self.output = self.output[:, :, :h_old * scale, :w_old * scale]
                # self.output = self.net_f(self.output1, align_feat)
                # self.output = self.output[:, :, :h_old * scale, :w_old * scale]
            self.net_g.train()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr=True, use_image=True):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.opt['is_train']:
                ...
                if 'gt' in visuals:
                    b, c, h, w = visuals['gt'].shape
                    store_vis_data(self.out_vis, '01_gt_rgb', self.gt, 'rgb', num_flow=1)
            ...
            if save_img:
                if self.opt['is_train']:
                    ...
                    # save predicted moiréd image
                    save_img_path = osp.join(self.opt['path']['visualization'], dataset_name,
                                             f'{img_name}.png')
                    imwrite_gt(sr_img, save_img_path)
                    
                    # save visualization tensor (include features)
