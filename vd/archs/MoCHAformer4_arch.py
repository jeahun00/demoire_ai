"""
MoCHAformer4: fca 는 0.01 로 고정 scale, fdb 는 0.01 로 초기화 한 후 learnable 하게
MoCHAformer5: fca 는 0.01 로 고정 scale, fdb 는 0.01 로 고정 scale
"""
import math
import numpy as np
import torch
...
class MoCHAformer4(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_frames=3,
                 **kwargs):
        super(MoCHAformer4, self).__init__()
    ...
    def forward(self, x, phase='train', is_train=True):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if c == 4:
                ...
                if self.stage == 1:
                    if vis_state:
                        store_vis_data(out_vis, '01_extraed_raw_feat', rearrange(x, '(b t) c h w -> b c t h w', b=n, t=t), 'raw', num_flow=1)
                if self.stage == 2:
                    ## vis part #######################################################################################
                    if vis_state:
