# Clean to Clean infernece 를 위한 datalaoder code
...
class MultiFrameVDPairedN2NC2CRawDataset(data.Dataset):
    def __init__(self, opt):
        super(MultiFrameVDPairedN2NC2CRawDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        img_lq_raw_0 = read_npz_16bit(lq_0_path)

        lq_1_path = self.paths[index]['lq_raw_1_path']
        img_lq_raw_1 = read_npz_16bit(lq_1_path)

        lq_2_path = self.paths[index]['lq_raw_2_path']
        img_lq_raw_2 = read_npz_16bit(lq_2_path)
