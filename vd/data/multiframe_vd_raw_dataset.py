...
class MultiFrameVDPairedRawDataset(data.Dataset):
    def __init__(self, opt):
        super(MultiFrameVDPairedRawDataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            self.paths = video_rawrgb_3frames_from_folders_train([self.lq_folder, self.gt_folder])
        else:
            self.paths = video_rawrgb_3frames_from_folders_val([self.lq_folder, self.gt_folder])
