...
class MultiFrameVDPairedN2NRaw_Test_Dataset(data.Dataset):
    def __init__(self, opt):
        super(MultiFrameVDPairedN2NRaw_Test_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            self.paths = n2n_for_test_train([self.lq_folder, self.gt_folder])
        else:
            self.paths = n2n_for_test_val([self.lq_folder, self.gt_folder])
